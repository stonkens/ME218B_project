...
ES_Event_t RunBallProcessingSM(ES_Event_t ThisEvent)
{
   ...
   switch (CurrentState)
  {
    case Waiting4Ball:
    {
      if (((ThisEvent.EventType == ES_BALL_DETECTED) && (ThisEvent.EventParam == BLUE)) || 
        ((ThisEvent.EventType == ES_BALL_DETECTED) && (ThisEvent.EventParam == RecycleColor)))
      {
      numRecycled++;
      printf("Recycrables %d\r\n", numRecycled);
      ES_Timer_InitTimer(PROCESSING_TIMER,PROCESSING_TIME); 
      CurrentState = Recycle;
      }else if(ThisEvent.EventType == ES_BALL_DETECTED){
      numTrashed++; 
      printf("Trash %d\r\n", numTrashed);
      ES_Timer_InitTimer(PROCESSING_TIMER,PROCESSING_TIME); 
    ...
    case Recycle:
    {
      if (ThisEvent.EventType == ES_TIMEOUT && ThisEvent.EventParam == PROCESSING_TIMER)
      {
        ...
        //when timer expires
      if(ThisEvent.EventType == ES_BALL_GONE){
        //reset cookie 
        resetCookie(); 
