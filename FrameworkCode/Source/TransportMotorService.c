...
// This module
#include "TransportMotorService.h"
...
#define TEST
static void InitTransportPWM(void); 
...
// start by selecting the alternate function for PB7
  HWREG(GPIO_PORTB_BASE + GPIO_O_AFSEL) |= (BIT7HI);
  //HWREG(GPIO_PORTB_BASE + GPIO_O_AFSEL) |= (BIT7HI | BIT6HI);
  
// now choose to map PWM to those pins, this is a mux value of 4 that we
// want to use for specifying the function on bits 7

  HWREG(GPIO_PORTB_BASE + GPIO_O_PCTL) =
      (HWREG(GPIO_PORTB_BASE + GPIO_O_PCTL) & 0x0fffffff) +
      (4 << (7 * BitsPerNibble));
...
int main(void)
{
  ...
  // is running.
  puts("\rStarting Transport Test Harness for \r");
